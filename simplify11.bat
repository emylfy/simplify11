...
for /f "usebackq delims=" %%i in (`powershell -Command "Get-ComputerRestorePoint | Where-Object { $_.Description -eq 'Pre-Script Restore Point' } | Measure-Object -Property Description | Select-Object -ExpandProperty Count"`) do (
    if %%i gtr 0 (
        ...
        echo %cMauve% '%cGrey% Pre-Script Restore Point already exists.               %cMauve%'%cReset%
        echo %cMauve% '%cGrey% You can apply it from the main menu.                   %cMauve%'%cReset%
        echo %cMauve% +--------------------------------------------------------+%cReset%
        pause
        goto main
    )
...
if %choice%==1 (
    goto lenovoMenu
) else if %choice% geq 0 if %choice% leq 8 (
    start "" "!url[%choice%]!"
    goto main
) else if %choice%==9 (
    goto main
